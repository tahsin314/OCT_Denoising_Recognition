...
valid_ds = OCTDataset(valid_df.image_name.values, valid_meta, valid_df.target.values, dim=sz, transforms=val_aug)
...
def evaluate():
   ...
   with torch.no_grad():
    for t in range(len(augs)):
      ...
      test_ds = OCTDataset(image_ids=valid_df.image_name.values, meta_features=valid_meta, dim=sz, transforms=augs[t])
...
def evaluatev2(test_df, test_meta):
   ...
   with torch.no_grad():
     for t in range(len(augs)):
      print('TTA {}'.format(t+1))
      test_ds = OCTDataset(image_ids=test_df.image_name.values, meta_features=test_meta, dim=sz, transforms=augs[t])
