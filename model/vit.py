...
class ViT(nn.Module):
    def __init__(self, pretrained_model='vit_base_patch16_384', num_class=5):
        ...
        OCTs = []
        for b in self.backbone.OCTs:
            OCTs.append(nn.Sequential(b))
        self.OCTs = nn.Sequential(*OCTs)
        # print(self.OCTs[-1][0].mlp.fc2.out_features)
        # print(self.backbone.norm)
        self.in_features = self.backbone.norm.normalized_shape[0]
        self.in_features = 576
        # print(self.in_features)
        # print([f"{b}\n\n\n New Layer" for b in self.backbone.OCTs])
